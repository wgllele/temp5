...
合约文档：[StablecoinBridgeTron](./StablecoinBridgeTron.md)（波场→ETH，只跨）· [StablecoinBridgeRouter](./StablecoinBridgeRouter.md)（ETH 只兑收费、ETH→波场）· [StablecoinBridgeQuery](./StablecoinBridgeQuery.md)（只读询价，失败返回 0）· [前端对接](./StablecoinBridgeRouter.frontend.md)· [UsdtOFT 通道详解](./UsdtOFT.md)· [审计回应](./StablecoinBridge.audit-response.md)
